...
#' p1 <- covplot(sobj, type = "relpos", facetting = FALSE)
#' p2 <- covplot(sobj, type = "rotation", facetting = FALSE)
#' p3 <- covplot(sobj, type = "relpred", facetting = FALSE)
#' gridExtra::grid.arrange(p1, p2, p3, ncol = 3)
#' @export

covplot <- function(sobj, type= "relpos", ordering = TRUE, facetting = TRUE) {
  ## ---- Warm up ----
  simtype <- sobj$get_parameters("type")
  m <- sobj$get_parameters("m")
  p <- sobj$get_parameters("p")
  if (type == "relpos") {
    lst <- sobj$get_properties("relpos")
    if (simtype == "bivariate") {
      comn <- do.call(intersect, lst)
      lst <- append(lapply(lst, function(x) setdiff(x, comn)), list(comn), after = 1)
    }
  } else {
    lst <- unname(sobj$get_properties("relpred"))
  ...
  if (simtype == "multivariate") {
    ...
    rot <- Reduce(rbind, rep(0, m),
                  Reduce(cbind, rep(0, m), sobj$get_properties("rotation_x"), right = TRUE),
                  right = TRUE)
    rot[1:m, 1:m] <- sobj$get_properties("rotation_y")
    ## Covariance Matrix
    mat <- if(type == "relpos") sobj$get_properties("sigma_zw") else sobj$get_properties("sigma")
    idx <- c(unlist(sobj$get_properties("ypos")), idx + m)
  } else {
    ## Rotation Matrix
    rotX <- sobj$get_properties("rotation_x")
    rot <- Reduce(rbind, rep(0, m),
                  Reduce(cbind, rep(0, m), rotX, right = TRUE),
                  right = TRUE)
    diag(rot)[1:m] <- 1
    ## Covariance Matrix
    sigma <- sobj$get_properties("sigma")
    sigma <- rot %*% sigma %*% t(rot)
    mat <- if(type == "relpos") sobj$get_properties("sigma") else sigma
    ## setting up index
    idx <- c(1:m, idx + m)
  }
  ...
  ## ypos map
  if (sobj$get_parameters("type") == "multivariate") {
    ypos <- vector("character", length = m)
    names(ypos) <- paste0(yvar, seq.int(m))
    for (x in sobj$get_parameters("ypos")) ypos[x] <- paste0(yvar, x[1])
  }
  
  for (row in seq_len(NROW(genmat))) {
    yvec <- if (sobj$get_parameters("type") == "multivariate") ypos[row] else paste0(yvar, row)
